...
public class RecyclerAdapter_select extends RecyclerView.Adapter<RecyclerAdapter_select.MyViewHolder> {

    private ArrayList<item> mList;
    private LayoutInflater mInflate;
    private Context mContext;
    public RecyclerAdapter_select(Context context, ArrayList<item> items) {
        this.mList = items;
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) { //뷰홀더가 재활용될 때 실행되는 메서드
        ...
        holder.txtContent.setText(mList.get(position).txtOneContent+"/"+mList.get(position).txtTwoContent+"/"+mList.get(position).txtThreeContent);
            ...
            itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) { // 아이템 클릭 시 !
                    ...
                    if (pos != RecyclerView.NO_POSITION) { //그 포지션이 recyclerView의 item을 클릭한 것인지 item이아닌 다른 클릭인지 여부를 확인한다.
                        Intent intent=new Intent(mContext, details_select.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
